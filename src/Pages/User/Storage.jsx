...
import {
  ...
  MdOutlineInventory2, MdHistory, MdSecurity,
} from 'react-icons/md';
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <div className="flex items-center gap-2.5 px-5 py-2 bg-gray-50 border-b border-gray-100">
        ...
        <span className="text-xs text-gray-500 font-medium">{isPickup ? 'Pickup Location' : 'Storage Location'}</span>
      </div>
...
const FacilityCard = ({ f }) => (
  <motion.div variants={cardVariants} whileHover={{ y: -4, boxShadow: '0 12px 32px rgba(0,0,0,0.10)' }}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="flex items-center gap-1.5 text-xs text-gray-500 mt-0.5 flex-wrap">
            ...
            <span className="text-gray-300">·</span><span>{f.distance}</span>
            <span className="text-gray-300">·</span><span>{f.size}</span>
          </div>
        </div>
      </div>
    ...
    <div className="flex flex-wrap gap-2 mb-5">
      {f.tags.map(t => <span key={t} className="px-2.5 py-1 text-xs font-medium text-blue-600 bg-blue-50 rounded-full border border-blue-100">{t}</span>)}
    </div>
    <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-auto">
      <div><p className="text-xl font-bold text-blue-600">{f.price}</p><p className="text-xs text-gray-400">starting from</p></div>
    ...
    <motion.div variants={pageVariants} initial="hidden" animate="show"
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}
        ...
        <div className="flex-1 flex flex-col w-full px-6 md:px-10 pt-8 pb-10 relative z-10">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Welcome to <span className="text-blue-600">Storage!</span></h1>
