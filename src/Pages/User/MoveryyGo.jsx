Moveryy - Web - Version / src / Pages / User / Rides.jsimport { useState } from 'react';
...
const MOCK_RIDERS = [
  {
    ...
    eta: '3 min',
    vehicle: 'Swift Dzire · DL 4C 2341',
    basePrice: 280,
    avatar: 'A',
    verified: true,
    color: 'bg-blue-500',
  },
  {
    ...
    eta: '5 min',
    vehicle: 'Honda City · MH 12 AB 9876',
    basePrice: 260,
    avatar: 'P',
    verified: true,
    color: 'bg-blue-500',
  },
  {
    ...
    eta: '7 min',
    vehicle: 'Maruti Ertiga · KA 05 MN 4512',
    basePrice: 240,
    avatar: 'R',
    verified: false,
    color: 'bg-blue-500',
  },
  {
    ...
    eta: '9 min',
    vehicle: 'Hyundai i20 · UP 32 GH 7823',
    basePrice: 220,
    avatar: 'S',
    verified: true,
    color: 'bg-blue-500',
...
// ── Rider Card ────────────────────────────────────────────────────────────────
const RiderCard = ({ rider, offeredPrice, onPriceChange, onSelect, isSelected }) => {
  ...
  const handleConfirm = () => {
    ...
    if (!isNaN(parsed) && parsed >= 50) {
      onPriceChange(rider.id, parsed);
    } else {
      setInputVal(String(offeredPrice));
    }
    setEditing(false);
  };

  const diff = offeredPrice - rider.basePrice;
  const diffLabel =
    diff === 0 ? null : diff > 0 ? `+₹${diff} above estimate` : `₹${Math.abs(diff)} below estimate`;
    ...
    >
      ...
      <div className="p-6">
        ...
        <div className="flex items-start gap-4 mb-5">
          <div className={`w-12 h-12 ${rider.color} rounded-2xl flex items-center justify-center text-white font-black text-lg flex-shrink-0 shadow-md`}>
            {rider.avatar}
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1 flex-wrap">
              ...
              <span className="text-xs text-slate-500">{rider.trips} trips</span>
              <span className="text-slate-300 text-xs">·</span>
              <span className="text-xs text-slate-500">{rider.distance}</span>
            </div>
            <div className="flex items-center gap-1.5 mt-1.5">
              <MdOutlineDirectionsCar size={14} className="text-slate-400" />
              <p className="text-xs text-slate-500 truncate">{rider.vehicle}</p>
            </div>
          </div>
        </div>

        {/* ETA + Price row */}
        <div className="flex items-center justify-between gap-4 bg-slate-50 rounded-2xl px-5 py-4">
          {/* ETA */}
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-black text-slate-800">{rider.eta}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-sm font-black text-slate-800">₹{rider.basePrice}</p>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {editing ? (
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  autoFocus
                  className="w-16 text-sm font-black text-slate-900 border-b-2 border-blue-400 outline-none bg-transparent"
                />
              </div>
            ) : (
              <button
                onClick={() => { setInputVal(String(offeredPrice)); setEditing(true); }}
            ...
            {diffLabel && (
              <p className={`text-[10px] font-semibold mt-0.5 ${diffColor}`}>{diffLabel}</p>
            )}
          </div>
        </div>
...
const MoveryyGoPage = () => {
  ...
  const [offeredPrices, setOfferedPrices] = useState(
    Object.fromEntries(MOCK_RIDERS.map((r) => [r.id, r.basePrice]))
  );
  ...
  const handlePriceChange = (riderId, price) => {
    setOfferedPrices((prev) => ({ ...prev, [riderId]: price }));
  };

  const handleSelect = (riderId) => {
    setSelectedRider((prev) => (prev === riderId ? null : riderId));
  };
  ...
  const selectedRiderData = MOCK_RIDERS.find((r) => r.id === selectedRider);
    ...
    >
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-12">
            ...
            <div className="flex flex-col gap-4 max-w-xl">
              ...
              <h1 className="text-4xl md:text-5xl font-extrabold text-white leading-tight tracking-tight">
                Moveryy Rides <br />
                <span className="text-yellow-400">Ride Smarter</span>
              </h1>
              ...
              <div className="bg-white/10 border border-white/20 rounded-3xl p-8 shadow-2xl space-y-6 min-w-[240px]">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-200">Avg. Rider Rating</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-200">Active Riders</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-14 pb-20">
          ...
          {searched && (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-2">
                <div>
                  <h3 className="text-xl font-black text-slate-900">Nearby Riders</h3>
                  <div className="w-12 h-1 bg-yellow-400 rounded-full mt-1" />
                </div>
                <span className="text-xs font-semibold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-full">
                  {MOCK_RIDERS.length} found within 3 km
                </span>
              </div>

              <p className="text-sm text-slate-500 mb-6 mt-2">
                Tap <span className="font-bold text-blue-600">✏ edit</span> on any offer price to negotiate your fare before selecting a rider.
              </p>
              ...
              >
                {MOCK_RIDERS.map((rider) => (
                  <RiderCard
                    key={rider.id}
                    rider={rider}
                    offeredPrice={offeredPrices[rider.id]}
                  ...
                  >
                    <div>
                      <p className="text-white font-black text-base">
                        ...
                        <span className="text-yellow-400 ml-2">· ₹{offeredPrices[selectedRider]}</span>
                      </p>
                      <p className="text-slate-400 text-xs mt-0.5">{selectedRiderData?.vehicle}</p>
                      <p className="text-slate-400 text-xs">{selectedRiderData?.eta} · {selectedRiderData?.distance}</p>
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0">
                      <a
                        href="tel:+919999999999"
                        className="flex items-center gap-2 bg-white/10 text-white px-4 py-2.5 rounded-2xl text-sm font-bold hover:bg-white/20 transition-colors"
                      >
                ...
                {booked && (
                  ...
                  >
                    ...
                    <p className="text-green-100 text-sm mb-1">
                      {selectedRiderData?.name} is on the way
                    </p>
                    <p className="text-white font-bold text-lg">
                      ₹{offeredPrices[selectedRider]} · {selectedRiderData?.eta}
                    </p>
                    <p className="text-green-200 text-xs mt-2">{selectedRiderData?.vehicle}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-12">
          {[
            { label: 'Active Riders', value: '10K+', bg: 'bg-blue-50', border: 'border-blue-100', text: 'text-blue-700' },
            { label: 'Rides Completed', value: '200K+', bg: 'bg-green-50', border: 'border-green-100', text: 'text-green-700' },
            { label: 'Avg. Rating', value: '4.9★', bg: 'bg-yellow-50', border: 'border-yellow-100', text: 'text-yellow-700' },
            { label: 'Cities', value: '50+', bg: 'bg-purple-50', border: 'border-purple-100', text: 'text-purple-700' },
          ].map((b) => (
        ...
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            {[
              { step: '01', title: 'Enter Route', desc: 'Add your pickup and drop location.', color: 'bg-blue-600' },
              { step: '02', title: 'Browse Riders', desc: 'See nearby riders with ETA and price estimates.', color: 'bg-yellow-500' },
              { step: '03', title: 'Negotiate Fare', desc: 'Edit the offer price to match your budget.', color: 'bg-orange-500' },
              { step: '04', title: 'Confirm & Ride', desc: 'Select your rider and confirm the booking.', color: 'bg-green-500' },
            ].map((s) => (
        ...
        <motion.div variants={fadeSlideUp} className="mt-10 bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {[
              { title: 'Negotiate Your Fare', desc: 'Set your own price — riders accept or counter.' },
              { title: 'Verified Riders Only', desc: 'Every rider is background-checked and verified.' },
              { title: 'Real-time Tracking', desc: 'Track your ride live from pickup to destination.' },
              { title: 'Safe & Insured', desc: 'All rides are insured for your peace of mind.' },
            ].map((f) => (
